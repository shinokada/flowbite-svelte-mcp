...
export const getComponentDocTool = {
    ...
    handler: async ({ component }) => {
        ...
        if (!indexRes.ok) {
            return { content: [{ type: "text", text: "Failed to load doc index" }], isError: true };
        }
        ...
        if (!matchLine) {
            return {
                content: [{ type: "text", text: `Component '${component}' not found in documentation index.` }],
                isError: true
            };
        }
        ...
        if (!docUrl) {
            return { content: [{ type: "text", text: "Invalid index entry found" }], isError: true };
        }
        // 4. Fetch the actual documentation
        const docRes = await fetch(docUrl);
        if (!docRes.ok) {
            return { content: [{ type: "text", text: `Failed to fetch content from ${docUrl}` }], isError: true };
        }
