...
const Schema = z.object({
  component: z.string().describe("The name of the component (e.g., 'accordion', 'checkbox')"),
});
...
export const getComponentDocTool = {
  ...
  handler: async ({ component }: ComponentDocArgs) => {
    ...
    if (!indexRes.ok) {
      return { content: [{ type: "text" as const, text: "Failed to load doc index" }], isError: true };
    }
    ...
    if (!matchLine) {
       return { 
         content: [{ type: "text" as const, text: `Component '${component}' not found in documentation index.` }],
         isError: true
       };
    }
    ...
    if (!docUrl) {
        return { content: [{ type: "text" as const, text: "Invalid index entry found" }], isError: true };
    }

    // 4. Fetch the actual documentation
    const docRes = await fetch(docUrl);
    if (!docRes.ok) {
      return { content: [{ type: "text" as const, text: `Failed to fetch content from ${docUrl}` }], isError: true };
    }
