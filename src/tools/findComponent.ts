...
export const findComponentTool = {
  definition: {
    name: 'findComponent',
    description:
      'Find a Flowbite-Svelte component and its documentation path. Use this FIRST before getComponentDoc to locate the correct component.',
    schema: Schema,
  },
  handler: async ({ query }: { query: string }) => {
    ...
    for (const [key, value] of Object.entries(componentsData)) {
      ...
      const matchingComponents = value.components.filter((comp) =>
